...
public class Painter : MonoBehaviour
{
    ...
    void Awake()
    {
        brush = GenerateBrush(brushIMG[brushSelection],
            areaOfEffectSizeZ,
            areaOfEffectSizeX); // This will take the brush image from our array and will resize it to the area of effect
        targetTerrain =
            FindObjectOfType<Terrain>(); // this will find terrain in your scene, alternatively, if you know you will only have one terrain, you can make it a public variable and assign it that way

        targetTerrain.terrainData = TerrainDataCloner.Clone(targetTerrain.terrainData);
        targetTerrain.GetComponent<TerrainCollider>().terrainData =
            targetTerrain.terrainData; // Don't forget to update the TerrainCollider as well
    ...
    private void SetEditValues(Terrain terrain)
    {
    ...
    private void GetTerrainCoordinates(RaycastHit hit, out int x, out int z)
    {
        int offset =
            areaOfEffectSizeX /
            2; //This offsets the hit position to account for the size of the brush which gets drawn from the corner out
        ...
        Vector3 terrainCoordinates = new Vector3(tempTerrainCoodinates.x / terrainSize.x,
        ...
        Vector3 locationInTerrain = new Vector3(terrainCoordinates.x * terrainHeightMapWidth,
            0,
    ...
    private float GetSurroundingHeights(float[,] height, int x, int z)
    {
        ...
        float
            avg = height[x,
                z]; // we will add all the heights to this and divide by int num bellow to get the average height
        int num = 1;
    ...
    public float[,] GetCurrentTerrainHeightMap()
    {
        if (targetTerrain)
        {
            ...
            return targetTerrain.terrainData.GetHeights(0,
                0,
    ...
    public float[,] GenerateBrush(Texture2D texture, int sizeX, int sizeZ)
    {
        ...
        Texture2D
            scaledBrush =
                ResizeBrush(texture,
                    sizeX,
                    sizeZ); // this calls a function which we will write next, and resizes the brush image
    ...
    public void SetBrushSize(int value) //adds int value to brush size(make negative to shrink)
    {
        ...
        brush = GenerateBrush(brushIMG[brushSelection],
            areaOfEffectSizeX,
            areaOfEffectSizeZ); // regenerates the brush with new size
    }
    ...
    void ModifyTerrain(int x, int z, float strength)
    {
        ...
        {
            heights = targetTerrainData.GetHeights(x - AOExMod,
                z - AOEzMod,
                areaOfEffectSizeX + AOExMod - AOExMod1,
                areaOfEffectSizeX + AOEzMod -
                AOEzMod1); // this grabs the heightmap values within the brushes area of effect
        }
        ...
        {
            splat = targetTerrain.terrainData.GetAlphamaps(x - AOExMod,
                z - AOEzMod,
                areaOfEffectSizeX + AOExMod,
                areaOfEffectSizeZ + AOEzMod); //grabs the splat map data for our brush area
            for (int xx = 0; xx < areaOfEffectSizeZ + AOEzMod; xx++)
            {
                for (int yy = 0; yy < areaOfEffectSizeX + AOExMod; yy++)
                {
                    float[]
                        weights = new float[targetTerrain.terrainData
    ...
    private Vector2Int GetTerrainPosition(Vector3 position)
    {
        Vector3 pos = position - targetTerrain.transform.position;
        Vector3 size = GetTerrainSize();
        Vector3 pos1 = new(pos.x / size.x, 0, pos.z / size.z);
        Vector3 locationInTerrain = new(pos1.x * terrainHeightMapWidth,
            0,
            pos1.z * terrainHeightMapHeight);

        return new Vector2Int((int)locationInTerrain.x - areaOfEffectSizeX / 2,
            (int)locationInTerrain.z - areaOfEffectSizeZ / 2);
    }
